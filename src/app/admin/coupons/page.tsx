...
export default function AdminCouponsPage() {
  ...
  const fetchCoupons = async () => {
    try {
      const res = await fetch("/api/admin/categories"); // reuse admin check pattern
      // Actually fetch coupons from a dedicated admin coupons endpoint
      const couponsRes = await fetch("/api/coupons?admin=true");
      if (couponsRes.ok) {
          ...
          <Card className="mb-6">
            ...
            <CardContent>
              <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label>Code</Label>
                  <Input placeholder="e.g. SAVE20" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} required />
                </div>
                <div>
                  <Label>Type</Label>
                  ...
                  <tbody>
                    {coupons.map((c: any) => (
                      <tr key={c.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-2">{c.type === "PERCENTAGE" ? "%" : "₹"}</td>
